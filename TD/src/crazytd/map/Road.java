...
public class Road extends Block{
	...
	public Road(double x, double y, String inDir, String outDir) {
		super(x, y);
		this.type = BlockType.valueOf("ROAD");
		setDir(true, inDir);
		setDir(false,outDir);
	}
	...
	public Vector getDirection(double x, double y){
		...
		if (xInTile > tileSize || xInTile<0 || yInTile > tileSize ||yInTile<0) return o; //out of boundary
		switch(inDirection){
		case NORTH:
			switch(outDirection){
			case SOUTH:
			...
			case EAST:	//either heading south or east
				if ((y-this.y)>=tileSize/2)	//over the half point on the way south, should turn now
					return e;
				else return s;
		...
		case SOUTH:
			...
			case EAST:	//either heading north or east
				if ((y-this.y)<=tileSize/2) //over the half point on the way north, should turn now
					return e;
				else return n;
			case WEST:
				if ((y-this.y)<=tileSize/2) //over the half point on the way north, should turn now
		...
		case EAST:
			...
			case NORTH:	//either heading east or north
				if ((y-this.y)>=tileSize/2) //over the half point on the way east, should turn now
					return n;
				else return w;
			case SOUTH:
				if ((y-this.y)>=tileSize/2) //over the half point on the way east, should turn now
